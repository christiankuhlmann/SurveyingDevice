    ...
    switch (current_mode)
    {
        ...
        if (action == ACTION_ON_SHORT)
        {
            ...
            if (!takeShot())
            {
                ...
                data.setDistance(sh.getShotData().d);
                data.setHeading(sh.getShotData().HIR(0));
                data.setInclination(sh.getShotData().HIR(1));
                data.setRoll(sh.getShotData().HIR(2));
                data.setTimestamp(millis());
                Debug_csd::debugf(Debug_csd::DEBUG_ALWAYS, "Shot taken: %f", data.getDistance());
                Debug_csd::debugf(Debug_csd::DEBUG_ALWAYS, "Heading: %f", data.getHeading());
                Debug_csd::debugf(Debug_csd::DEBUG_ALWAYS, "Inclination: %f", data.getInclination());
                Debug_csd::debugf(Debug_csd::DEBUG_ALWAYS, "Roll: %f", data.getRoll());
                Debug_csd::debugf(Debug_csd::DEBUG_ALWAYS, "Timestamp: %d", data.getTimestamp());
                sendBLEData(data);
            };
